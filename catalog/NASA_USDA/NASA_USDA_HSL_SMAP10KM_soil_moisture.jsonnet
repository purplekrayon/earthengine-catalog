...

{
  ...
  description: |||
    ...
    Level 2 soil moisture observations into the modified two-layer Palmer model using a 1-D
    ...
    This dataset was developed by the Hydrological Science Laboratory at NASA's Goddard Space Flight 
    Center in cooperation with USDA Foreign Agricultural Services and USDA Hydrology and Remote Sensing 
    Lab. As of August 2022, the NASA-USDA Enhanced SMAP global soil moisture product has been discontinued. 
    However, there are comparable SMAP-based soil moisture products that are recommended. 
    
    + The Level-3 (L3) [SPL3SMP_E.005 soil moisture product](NASA_SMAP_SPL3SMP_E_005) provides a daily composite 
    of global land surface conditions retrieved by the Soil Moisture Active Passive (SMAP) L-Band radiometer.
    
    + The SMAP Level-4 (L4) [SPL4SMGP soil moisture product](NASA_SMAP_SPL4SMGP_007) includes surface and root-zone soil 
    moisture, as well as additional research products. Please note, the SPL4SMGP soil 
    moisture estimates are obtained by merging SMAP observations with estimates from the Catchment Land Surface 
    Model in a soil moisture data assimilation system. The product is 3-hourly instead of daily (see table below).
